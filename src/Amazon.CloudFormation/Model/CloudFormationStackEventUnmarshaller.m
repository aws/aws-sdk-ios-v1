...
@implementation CloudFormationStackEventUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
	...
	if ([elementName isEqualToString:@"ResourceProperties"]) {
        self.response.resourceProperties = self.currentText;
        return;
    }
    ...
    if ([elementName isEqualToString:@"StackEvent"]) {
        if (caller != nil) {
...
-(CloudFormationStackEvent *)response
{
    if (nil == response) {
        response = [[CloudFormationStackEvent alloc] init];
    }
    return response;
}
