...
@implementation CloudFormationValidateTemplateResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
	...
	if ([elementName isEqualToString:@"Parameters"]) {
        ...
        listUnmarshaller.entryElementName   = @"member";
        listUnmarshaller.delegateClass      = [CloudFormationTemplateParameterUnmarshaller class];
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
	...
	if ([elementName isEqualToString:@"CapabilitiesReason"]) {
        self.response.capabilitiesReason = self.currentText;
        return;
    }
    ...
    if ([elementName isEqualToString:@"ValidateTemplateResult"]) {
        if (caller != nil) {
...
-(CloudFormationValidateTemplateResponse *)response
{
    if (nil == response) {
        response = [[CloudFormationValidateTemplateResponse alloc] init];
    }
    return response;
}
